...
def fetch_code(repo_url: str, path: str="") -> str:
    ...
    try:
        ...
        return f" error fetching code is {e}"
         

#agent functions
def planner_agent(state: AgentState) -> AgentState:
    """Decides which steps to execute next base on what's already done.
    """
    ...
    - Security issue found: {bool(state['security_issues'])}
    ...
    print(f"Planner decide next action: {next_action}")
...
def security_review(state: AgentState) -> AgentState:
    ...
    prompt = f"""
    Analyze this java code for security vulnerabilities.PermissionError
...
# conditional branching - true "agentic" behaviour
